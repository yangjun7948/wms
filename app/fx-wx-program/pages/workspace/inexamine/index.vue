<template>
	<view>
		<cu-custom bgColor="bg-cyan" :isBack="true">
			...
			<block slot="content">入库单审核</block>
		</cu-custom>
		<view class="padding">
	...
	export default {
		...
		data() {
			return {
				...
				loadStatus: "loadmore",
				orderList: [],
				getInReceiptList: null
			}
		},
		onLoad() {
			let that = this
			this.getInReceiptList = debounce(() => {
				that._getInReceiptList();
			}, 1000, true);
		},
		created() {
		...
		methods: {
			resetList() {
				...
				this.orderList = [];
				this._getInReceiptList();
			},
			_getInReceiptList() {
				var that = this;
				if (that.loadStatus == "loadmore") {
					let opts = {
						url: 'warehouse/app/inReceiptApproveList?pageNum=' + that.pageNum + '&pageSize=' + that
							.pageSize,
						method: 'get'
					};
			...
			godetails(item) {
				let params = {
					receiptId: item.warehouseInReceiptId,
				}
