	...
	export default {
		...
		methods: {
			resetList() {
				...
				this.orderList = [];
				this._getInReceiptScanList();
			},
			_getInReceiptScanList() {
				var that = this;
				if (that.loadStatus == "loadmore") {
					let opts = {
						url: 'warehouse/app/inReceiptScanList?pageNum=' + that.pageNum + '&pageSize=' + that
							.pageSize,
						method: 'get'
					};
			...
			godetails(item) {
				let params = {
					...
					"inWarehouseTime": item.inWarehouseTime,
					"remark": !!item.remark ? item.remark : "",
					"inWarehouseTypeName": item.inWarehouseTypeName,
					"warehouseInReceiptId": item.warehouseInReceiptId
				}
