						...
						<view class="flex align-center">
							<view style="flex:9;">
								<view class="text-lg margin-bottom-xl">
									<view class="flex align-center">
										<uni-text class="cuIcon-titles text-orange"></uni-text>
	...
	export default {
		...
		data() {
			return {
				...
				loadStatus: "loadmore",
				orderList: [],
				getOutReceiptScanList: null
			}
		},
		onLoad() {
			let that = this
			this.getOutReceiptScanList = debounce(() => {
				that._getOutReceiptScanList();
			}, 1000, true);
		},
		created() {
		...
		methods: {
			resetList() {
				...
				this.orderList = [];
				this._getOutReceiptScanList();
			},
			_getOutReceiptScanList() {
				var that = this;
				if (that.loadStatus == "loadmore") {
					let opts = {
						url: 'warehouse/app/outReceiptScanList?pageNum=' + that.pageNum + '&pageSize=' + that
							.pageSize,
						method: 'get'
					};
			...
			godetails(item) {
				let params = {
					...
					"outWarehouseTime": item.outWarehouseTime,
					"remark": !!item.remark ? item.remark : "",
					"outWarehouseTypeName": item.outWarehouseTypeName,
					"warehouseOutReceiptId": item.warehouseOutReceiptId
				}
