package com.xiaoyai.web.controller.system;
...
import com.xiaoyai.common.constant.UserConstants;
...
import com.xiaoyai.system.service.ISysDeptService;
import com.xiaoyai.system.service.ISysPostService;
import com.xiaoyai.system.service.ISysRoleService;
...
public class SysUserController extends BaseController {
     ...
     */
    @PreAuthorize("@ss.hasPermi('system:user:query')")
    @GetMapping("/authWarehouse/{userId}")
    public AjaxResult authWarehouse(@PathVariable("userId") Long userId) {
        AjaxResult ajax = AjaxResult.success();
        SysUser user = userService.selectUserById(userId);
        List<Long> warehouseIds = warehouseManagerService.selectWarehouseIdsByUserId(userId);
        List<Warehouse> warehouseList = warehouseService.list(Wrappers.<Warehouse>lambdaQuery().eq(
                Warehouse::getDelFlag, Constants.SUCCESS));
        ajax.put("user", user);
        ajax.put("userWarehouseIds", warehouseIds);
        ajax.put("warehouseList", warehouseList);
        return ajax;
    }
    ...
    @PreAuthorize("@ss.hasPermi('system:user:edit')")
    @Log(title = "用户管理", businessType = BusinessType.GRANT)
    @PutMapping("/authWarehouse")
    public AjaxResult insertAuthWarehouse(Long userId, Long[] warehouseIds) {
        warehouseManagerService.remove(Wrappers.<WarehouseManager>lambdaQuery().eq(WarehouseManager::getUserId, userId));
        warehouseManagerService.saveBatch(Arrays.stream(warehouseIds).map(t -> {
            WarehouseManager warehouseManager = new WarehouseManager();
            warehouseManager.setWarehouseId(t);
            warehouseManager.setUserId(userId);
    ...
    public AjaxResult deptUserTree() {
        return AjaxResult.success(userService.deptUserTree());
    }
}
